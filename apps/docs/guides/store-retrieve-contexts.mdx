...
// by version id — immutable, the address to store (preferred)
const { data } = await uc.get(contextId, { version: 'ctx_4b7e0d' });

// by version number — deprecated positional alias (-1 = latest)
...
// → versions: [
//    { version: 0, id: 'ctx_9f2a1c', operation: 'create', affected: null, created_at: '...' },
//    { version: 1, id: 'ctx_4b7e0d', operation: 'update', affected: ['msg_abc'], created_at: '...' },
// ]
```
...
# → result["versions"]: [
#     { "version": 0, "id": "ctx_9f2a1c", "operation": "create", "affected": None, "created_at": "..." },
#     { "version": 1, "id": "ctx_4b7e0d", "operation": "update", "affected": ["msg_abc"], "created_at": "..." },
#   ]
```
...
Then you can retrieve the state at a specific version — by its immutable `id`
(the address to store; it never changes meaning) or by its positional `version`
number (a deprecated alias that shifts as the chain grows):

<CodeGroup>
```typescript TypeScript
const { data } = await uc.get(contextId, { version: 'ctx_4b7e0d' }); // by id
const { data } = await uc.get(contextId, { version: 1 });            // by index
```

```python Python
result = uc.get(context_id, version="ctx_4b7e0d")  # by id
result = uc.get(context_id, version=1)             # by index
```
