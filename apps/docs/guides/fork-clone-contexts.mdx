...
// by version id — immutable address (preferred)
const fork = await uc.create({ from: 'ctx_abc123', version: 'ctx_4b7e0d' });
...
# by version id — immutable address (preferred)
fork = uc.create(from_="ctx_abc123", version="ctx_4b7e0d")
...
`version` takes an immutable version id (`ctx_…`) or a positional index; prefer the id, because indexes shift as the chain grows.
</Info>
