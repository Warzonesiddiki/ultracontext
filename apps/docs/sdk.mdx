...
// → versions: [
//     { version: 0, id: 'ctx_9f2a1c', operation: 'create', affected: null },
//     { version: 1, id: 'ctx_4b7e0d', operation: 'update', affected: ['msg_a'] },
//     ...
// ]
// `id` is the immutable address — store that, not the positional `version`

// name a version so the reference is readable and stable
await uc.setBranch(ctx.id, { name: 'before-refactor', version: 'ctx_4b7e0d' });
const { branches } = await uc.branches(ctx.id);
...
# → result["versions"]: [
#     {"version": 0, "id": "ctx_9f2a1c", "operation": "create", "affected": None},
#     {"version": 1, "id": "ctx_4b7e0d", "operation": "update", "affected": ["msg_a"]},
#     ...
# ]
...
Every update and delete auto-creates a new version. Retrieve any previous state with a single call, by an immutable version `id` that never changes meaning — or give that version a name with `setBranch()` / `branches()` / `deleteBranch()`.
